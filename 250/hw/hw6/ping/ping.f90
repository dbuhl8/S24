  ...
  do while (i /= 7)
    if (mod(i,2) == 0) then
      if (id == 0) then 
        ...
        print *, "Hey I'm processor ",id," and I've just sent ",i,&
          " TO   processor ",dest," on msg :", msgid
        msgid = msgid + 1
      else 
        call MPI_RECV(i, 1, MPI_INTEGER, src, msgid, MPI_COMM_WORLD, stat, ie)
        print *, "Hey I'm processor ",id," and I've just recv ",i,&
          " FROM processor ",src," on msg :", msgid
        msgid = msgid + 1
      end if
    else 
      if (id == 1) then 
